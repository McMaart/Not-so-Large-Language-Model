...
class TransformerModel(nn.Module):
    ...
    @torch.no_grad()
    def generate_tokens(self, start_token: Tensor, length: int, eos = None) -> Tensor:
        self.eval()
        token_tensor = start_token
        for _ in range(length):
            pred1 = self(token_tensor)[:, -1, :-num_spec_non_eos_tokens]
            probs = F.softmax(pred1, dim=-1)
            pred = torch.multinomial(probs, 1)
            token_tensor = torch.cat((token_tensor, pred), 1)
            if pred.item() == eos or token_tensor.size(1) == max_seq_len:
                return token_tensor
        return token_tensor
