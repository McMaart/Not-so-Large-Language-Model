...
class TransformerModel(nn.Module):
    ...
    def forward(self, x: Tensor, lengths: Tensor | None = None) -> Tensor:
        ...
        if self.pos_enc_type == 'rope':
            ...
            embedding = self.pos_encoding(embedding.view(embedding.size(0), embedding.size(1), self.nhead, -1))
...
if __name__ == '__main__':
    ...
    nhead = 8
    num_layers = 6
    model = TransformerModel(vocab_size=vocab_size, embed_size=embed_size, nhead=nhead, num_layers=num_layers,
                             pos_enc_type='rope')
    print(model)
